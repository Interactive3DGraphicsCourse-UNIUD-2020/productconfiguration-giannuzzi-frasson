...
void main() {
    ...
    vec3 outRadiance = ((PI * clight * nDotl * BRDF) + (PI * clight * nDotl1 * BRDF1) + (PI * clight * nDotl2 * BRDF2) + ambientLight) * cdiff * texture2D(aoMap, uVv * textureRepeat).xyz;
    // gamma encode the final value
    gl_FragColor = vec4(pow(outRadiance, vec3(1.0 / 2.2)), 1.0);
    }
`
...
function fragmentShaderPlastic() {
    return `varying vec3 vNormal;
			...
			void main() {
                ...
                vec3 BRDF2 = (vec3(1.0)-fresnel2)*cdiff/PI + fresnel2*GSmith(nDotv,nDotl2)*DGGX(nDoth2,roughness*roughness)/
					(4.0*nDotl2*nDotv);
				vec3 outRadiance = (PI* clight * nDotl * BRDF) + (PI* clight * nDotl1 * BRDF1) + (PI* clight * nDotl2 * BRDF2);
				// gamma encode the final value
				gl_FragColor = vec4(pow( outRadiance, vec3(1.0/2.2)), 1.0);
            }
            `
